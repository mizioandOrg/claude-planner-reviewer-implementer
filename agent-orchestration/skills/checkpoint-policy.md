...
### 4. Loop exit without approval (always)
If the loop ends because max iterations were reached without a score of 10/10, write `checkpoints/team_log.md` with the full team_log regardless of visibility setting. This ensures the user always has an inspectable artifact when the loop fails to converge.
...
- `plan.md` or `critique.md` unless `Visibility: high` is explicitly set in `problem.md`
- Any files outside `checkpoints/`, `plan.md`, and `critique.md`
